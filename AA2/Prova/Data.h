...
struct Player{
    ...
    void Move(const std::string &dir, Map &field){
        ...
        if(field.squares[pos.x][pos.y]== 'G') {

        std::cout << "GEM PICKED!" << std::endl;
        field.squares[pos.x][pos.y]=' ';

        // ACCESO A BASE DE DATOS
        // COMPARTIR EL VECTOR DE GEMAS DESDE EL PLAYER A CADA ITERACIÓN?

            //Gem gem; std::string name = "Joe";
            //sf::CircleShape shape;
            //gem.Init(rand()%15, name, shape);
            //gems.push_back(gem);
